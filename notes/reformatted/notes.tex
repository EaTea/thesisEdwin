...
\begin{proof}
We use an inductive proof on $mod(\phi)$, the modal depth of $\phi$.
Our inductive hypothesis is as follows: supposing that $mod(\phi) = k$ then
$M^1_{(s,t}) = M_s \otimes N_t \models \phi \iff M^2_{(s',t)} = M'_{s'} \otimes
N_t \models \phi$.\\
\\
For our base case, suppose $mod(\phi) = 0 \implies \phi \in \langProp$ and let
$M^1_{(s,t)} \models \phi$.
\begin{align}
  & \implies \phi \in V^1_((s,t)) \label{baseOne} \\
  & \implies \phi \in V(s) \label{baseTwo} \\
  & \implies \phi \in V'(s) \label{baseThree} \\
  & \implies \phi \in V^2((s',t)) \label{baseFour} \\
  & \implies M^2_{(s',t)} \models \phi \label{baseFive}
\end{align}
(\ref{baseOne}) follows from the definition of a Kripke model (Definition
\ref{model}). (\ref{baseTwo}) follows from the definition of event model execution
(Definition \ref{evModelEx}). (\ref{baseThree}) follows from hypothesis.
...
\\
Now, suppose our induction hypothesis holds for $mod(\phi) = k < n$.
Let $mod(\phi) = k+1$ and let $M^1_{(s,t)} \models \phi$.
Now, without loss of generality, let $a \in A$ and $\phi = \Box_a \psi$ or $\phi = \Diamond_a \psi$,
$mod(\psi) = k$.\\
\\
Let us suppose that $\phi = \Box_a \psi$.

\begin{align}
	& \implies M^1_{(s,t)} \models \Box_a \psi \label{nBisKriIn1A} \\
	& \implies M^1_{(u,v)} \models \psi \text{ for each $(u,v) \in (s,t) R^1_a$} \label{nBisKriIn2A} \\
	& \implies M^1_{(u,v)} \models \psi \text{ for each $u \in s R_a$ and $v \in t R^N_a$ and $M_{u}
		\models pre^N(v)$} \label{nBisKriIn3A} \\
	& \implies M_u \models \psi \text{ for each $u \in s R_a$ such that there is some $v \in t
		R^N_a$ and $M_u \models pre^N(v)$} \ref{nBisKriIn4A} \\
	& \implies M'_{u'} \models \psi \text{ for each $u' \in s' R'_a$ such that there is some $v \in t
		R^N_a$ and $M_{u'} \models pre^N(v)$} \ref{nBisKriIn5A} \\
	& \implies M'_{(u',v)} \models \psi \text{ for each $u' \in s' R'_a$ and $v \in t R^N_a$ and
		$M'_{u'} \models \pre^N(v)$} \ref{nBisKriIn6A} \\
	& \implies M'_{(s',t)} \models \Box_a \psi \ref{nBisKriIn7A} \\
	& \implies M'_{(s',t)} \models \phi \ref{nBisKriIn8A}
\end{align}
(\ref{nBisKriIn2A}) follows from the definition of $\Box_a$ in the semantics of modal logic (Lemma \ref{modalLogic}).
(\ref{nBisKriIn3A}) follows from the definition of event model execution (Definition
\ref{evModelEx}).
(\ref{nBisKriIn4A}) follows from the definition of positive formulae (Definition \FIXME).
(\ref{nBisKriIn5A}) follows from $mod(\psi) = k < n$, each point $M_u$ being $n-1$-bisimilar to some
point $M'_{u'}$ (by {\bf $n$-forth-a}) and thus $M_u \models \psi \iff M'_{u'} \models \psi$
(Definition \ref{nBisimKripke}).
(\ref{nBisKriIn6A}) follows from the definition of event model execution (Definition \ref{evModelEx}).
(\ref{nBisKriIn7A}) follows from the definition of $\Box_a$ (Lemma \ref{modalLogic}).\\
\\
Showing the converse uses a similar process.
A similar argument follows for when $\phi = \Diamond_a \psi$.
\end{proof}
...
\begin{proof}
We will inductively show that given $M_s \sim_n M'_{s'}$ then for a formula $\phi$ with modal depth
$mod(\phi) = k \leq n$, $M^1_{(t,s)} = N_t \otimes M_s \models \phi \iff M^2_{(t,s')} = N_t \otimes
M'_{s'} \models \phi$.\\
\\
We begin with a base case of $mod(\phi) = k = 0$.
Then $\phi \in \langProp$.
Suppose $M^1_{(t,s)} \models \phi$.
Then
\begin{align}
	& \implies \phi \in V^1_{(t,s)} \label{nBisEvbase1} \\
	& \implies \phi \in V(t) \label{nBisEvbase2} \\
	& \implies N_t \models pre(s) \label{nBisEvbase4} \\
	& \implies N_t \models pre'(s') \label{nBisEvbase5} \\
	& \implies \phi \in V^2_{(t,s')} \label{nBisEvbase6}\\
...
\ref{propLogic}).\\
\\
Now, suppose our induction hypothesis holds for any $\phi$ such that $mod(\phi) = k$.
Let $mod(\phi) = k+1$.
Without loss of generality, let $a \in A$ and $\phi = \Box_a \psi$ or $\phi = \Diamond_a \psi$,
where $mod(\psi) = k$.\\
\\
Suppose $\phi = \Box_a \psi$.
Then
\begin{align}
	& \implies M^1_{(t,s)} \models \Box_a \psi \label{nBisEvInd1A} \\
	& \implies M^1_{(u,v)} \models \psi \text{ for each $(u,v)  \in (t,s) R^1_a$} \label{nBisEvInd2A} \\
	& \implies M^1_{(u,v)} \models \psi \text{ for each $u \in t R^N_a$ and $v \in s R_a$ and $M_v
		\models pre(u)$} \label{nBisEvInd3A} \\
	& \implies M^1_{(u,v)} \models \psi \text{ for each $u \in t R^N_a$ and $v \in s R_a$ and $M_v
		\models pre(u)$} \label{nBisEvInd4A} \\
\end{align}
