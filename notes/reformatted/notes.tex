...
\begin{align}
	...
	& \vdash \bigwedge_{v \in T^\gamma} \sqex{M^\gamma_v} \gamma \text{ for every }\gamma \in \Gamma\\
	& \vdash \Box \bigwedge_{v \in T^\gamma} \sqex{M^\gamma_v} \gamma \text{ for every } \gamma \in \Gamma\\
	& \vdash \bigwedge_{v \in T^\gamma} \Box \sqex{M^\gamma_v} \gamma \text{ for every } \gamma \in \Gamma\\
	& \vdash \bigwedge_{\gamma \in \Gamma} \bigwedge_{v \in T^\gamma} \Box \sqex{M^\gamma_v} \gamma\\
	& \vdash \bigwedge_{\gamma \in \Gamma} \bigwedge_{v \in T^\gamma} \Box \sqex{M^\gamma_v}
	\bigvee_{\gamma' \in \Gamma} \gamma'\\
	& \vdash \bigwedge_{v \in \sigma R} \Box \sqex{M^\gamma_v}\bigvee_{\gamma' \in \Gamma} \gamma'\\
	& \vdash pre(\sigma) \implies \bigwedge_{v \in \sigma R} \Box \sqex{M_v} \bigvee_{\gamma' \in \Gamma} \gamma'\\
	& \vdash \sqex{M_\sigma} \Box \bigvee_{\gamma' \in \Gamma} \gamma'
\end{align}
...
\begin{align}
	...
	M^\gamma_{T^{\gamma}} } \gamma\\
	& \vdash \exists \phi \implies \bigwedge_{\gamma \in \Gamma} \Diamond \anex{
	\bigvee_{v \in T^\gamma} M^\gamma_{v} } \gamma\\
	& \vdash \exists \phi \implies \bigwedge_{\gamma \in \Gamma} \Diamond \anex{
	\bigvee_{v \in \sigma R} M^\gamma_{v} } \gamma\\
	& \vdash \bigwedge_{\gamma \in \Gamma} (\exists \phi \implies \bigvee_{v \in \sigma R}
	\Diamond \anex{ M^\gamma_{v} } \gamma)\\
...
\begin{align}
	...
	\bigwedge_{\gamma \in \Gamma} \sqex{M_\sigma} \Diamond \gamma \\
	& \vdash \sqex{M_\sigma}(\pi \land \cover \Gamma)
\end{align}

Lastly, we show that $\vdash \anex{ M_T } \phi \iff \exists \phi$.

\begin{align}
	& \vdash \anex{ M_\sigma } \phi \iff (pre(\sigma) \land [ M_\sigma ] \phi\\
	& \vdash \anex{ M_\sigma } \phi \iff pre(\sigma)\\
	& \vdash \anex{ M_\sigma } \phi \iff \exists \phi
\end{align}
...
finite forest event models.
Thus each action point in $M_T$ is bisimilar to a corresponding point in $M^\alpha_{T^\alpha}$ or
$M^\beta_{T^\beta}$.
Since each of $M^\alpha_{T^\alpha}$ and $M^\beta_{T^\beta}$ are finite forest event models and no
new accessibilities are added to $R$.
Thus $M_T$ is a finite forest event model.\\
\\
Next, we show that $\vdash \sqex{M_T}\phi$.
\begin{align}
	&\vdash \sqex{M^\alpha_{T^\alpha}}\alpha \land \sqex{M^\beta_{T^\beta}}\beta\\
	&\vdash \sqex{M_{T^\alpha}}\alpha \land \sqex{M_{T^\beta}}\beta\\
	&\vdash \sqex{M_{T^\alpha}}(\alpha \lor \beta) \land \sqex{M_{T^\beta}}(\alpha \lor \beta)\\
\end{align}

\begin{align}
	&\vdash \exists (\alpha \lor \beta) \implies (\exists \alpha \lor \exists \beta) \\
	&\vdash \exists (\alpha \lor \beta) \implies (\anex{ M^\alpha_{T^\alpha} } \alpha \lor
			\anex{ M^\beta_{T^\beta} }\beta) \\
	...
	&\vdash \exists (\alpha \lor \beta) \implies (\anex{M_{T^\alpha}} (\alpha \lor \beta) \lor \anex{M_{T^\beta}}
			(\alpha \lor \beta))\\
	&\vdash \exists (\alpha \lor \beta) \implies (\anex{M_T} (\alpha \lor \beta))
\end{align}
\end{proof}
