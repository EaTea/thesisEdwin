...
\\
Let us suppose that $a \in A$ and $\phi = \Box_a \psi$, where $mod(\psi) = k$.
\begin{align}
  & \implies M_t \models \psi \text{for each $t \in s R_a$} \label{inductOneA} \\
  & \implies M'_{t'} \models \psi \text{for each $t' \in s' R'_a$} \label{inductTwoA} \\
  & \implies M'_{s'} \models \psi \label{inductThreeA}
\end{align}
(\ref{inductOneA}) follows from event model execution preserving sentences
before execution, (\ref{inductTwoA}) follows from hypothesis and {\bf
$n$-forth-$a$} (Definition \ref{nBisimKripke}) and (\ref{inductThreeA}) follows from the definition of $\Box_a$
(Definition \ref{modalLogic}).

Let us suppose that $a \in A$ and $\phi = \Diamond_a \psi$, where $mod(\psi) = k$.
\begin{align}
  & \implies M_t \models \psi \text{for some $t \in s R_a$} \label{inductOneB} \\
  & \implies M'_{t'} \models \psi \text{for each $t' \in s' R'_a$} \label{inductTwoB} \\
  & \implies M'_{s'} \models \psi \label{inductThreeB}
\end{align}
(\ref{inductOneB}) follows from event model execution preserving sentences
before execution, (\ref{inductTwoB}) follows from hypothesis and {\bf
$n$-forth-$a$} (Definition \ref{nBisimKripke}) and (\ref{inductThreeB}) follows from the definition of $\Diamond_a$
(Definition \ref{modalLogic}).\\
\\
A similar argument follows for the converse, except it uses {\bf $n$-back-$a$}.
Thus we have $M^1_{(s,t)} \models \phi \iff M^2_{(s',t)} \models \phi$.
\end{proof}
