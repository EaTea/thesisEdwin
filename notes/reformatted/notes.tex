...
Suppose that for each $\gamma \in \Gamma$ there is a finite forest event model $M^\gamma_T \in \mathcal{AM}$ such that
...
Then there exists a finite forest event model $M_T \in \mathcal{AM}_B$ such that $\vdash \sqex{M_T}\phi$ and
...
Let $\gamma \in \{\alpha, \beta\}$ and let $N_s \in \kripkeClass$.
Let $M^\gamma_t$ be a subtree of $M^\gamma$, where $t \in T^\gamma$, and by hypothesis, $M^\gamma_t \in \eventClass_B$.
Suppose $\sigma \in \Sigma^\gamma_t$, the event points in the subtree $M^\gamma_t$.
As $M^\gamma_\sigma \in \eventClass_B$ then $N_s \otimes M^\gamma_\sigma \lesssim_B N_s$.
As $M$ is formed from the disjoint union of $M^\alpha$ and $M^\beta$ (which by Lemma FIXME are
disjoint unions of each of the finite subtrees $M^\gamma_t$), it is a simple matter to show that $M_\sigma \sim M^\gamma_\sigma$.
From Lemma \ref{eventModelEquivalent} we have that $N_s \otimes M_t \sim N_s \otimes
M^\gamma_\sigma$, which indictates that $N_s \otimes M_\sigma \lesssim_B N_s \otimes
M^\gamma_\sigma$ (by Lemma FIXME).
From Lemma FIXME, the relation $\lesssim_B$ is transitive, and thus $N_s \otimes M_\sigma \lesssim_B
N_s$, and so $M \in \eventClass_B$.\\
\\
