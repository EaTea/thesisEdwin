...
\\
To show that $M_T$ is a finite tree event model, we observe the following.
Firstly, $M_T$ is a pointed event model, with its point at $\sigma$.
$\Sigma$ is a finite set and the point $\sigma$ is inaccessible from any other point in $\Sigma$.\\
\\
Furthermore, let $\sigma_k$ be some point in $\Sigma \setminus \{ \sigma \}$.
Then $\sigma_k \in M^{\gamma_k}_{t^{\gamma_k}}$ where $M^{\gamma_k}_{t^{\gamma_k}}$ is the finite
tree event model corresponding to $\gamma_k \in \Gamma$ and $t^{\gamma_k} \in T^{\gamma_k}$.
We observe that by the definition of finite tree event models there exists a unique sequence
$\sigma^{1}_{k}, \sigma^2_{k}, \ldots, \sigma^d_k \in \Sigma^{\gamma_k}_{t^{\gamma_k}}$ such that
$t^{\gamma_k} R \sigma^1_k$, $\sigma^1_k R \sigma^2_k$, $\ldots$, $\sigma^{d-1}_k R \sigma^d_k$,
$\sigma^d_k R \sigma_k$.
Thus $M^{\gamma_k}_{t^{\gamma_k}}$ is a finite tree event model with its point $t^{\gamma_k} \in
T^{\gamma_k}$.
By the definition ot $\to$, $(\ldots (M'_\sigma \to M^{\gamma_1}_{T^{\gamma_1}}) \to \ldots ) \to
M^{\gamma_k}_{T^{\gamma_k}} \implies \sigma R t^{\gamma_k}$.
Then there is a unique path from $\sigma$ to $\sigma_k$.
Thus $M_T$ is a finite tree event model and is also a finite forest event model.\\
\\
