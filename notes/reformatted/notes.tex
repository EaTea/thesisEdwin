...
Our inductive hypothesis is as follows: supposing that $mod(\phi) = k$ then $M_s
\otimes N_t \models \phi \iff M'_{s'} \otimes N_t \models \phi$.\\
\\
For our base case, suppose $mod(\phi) = 0$.
Then $\phi \in \langProp$.
By the definition of $n$-bisimilarity (Definition \ref{nBisimKripke}) $M_s
\sim_n M'_{s'} \implies M_s \sim_0 M'_{s'} \implies V(s) = V'(s')$.
\end{proof}
