...
Then there exists a finite tree-like event model $M \in \mathcal{AM}_B$ such that
$[M]\phi$ and $\langle M \rangle \phi \iff \exists_B \phi$.
\end{thm}

\begin{lemma}
Let $B \subseteq A$ and let $\phi = \alpha \lor \beta \in mathcal{L}$.
Then there exists a finite tree-like event model $M \in \mathcal{AM}_B$ such that $[M]\phi$ and
$\langle M \rangle \phi \iff \exists_B \phi$.
\end{lemma}
