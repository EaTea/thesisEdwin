...
\begin{itemize}
	\item $\Sigma' = \{(s,u) | s \in \Sigma, u \in \Sigma^N$
\end{itemize}
\end{defn}
...
We will discuss the single-agent arbitrary action model logic.
Thus $A$ contains a single agent $a$ and our modal operators will only involve that agent for this
chapter.
...
\[
	\phi :== p | \not \phi | (\phi \land \phi) | \box \phi | [ M ] \phi | \forall \phi
\]
where $p$ is an atomic statement and $M \in \mathcal{AM}$ is a multi-pointed event model with
...
Suppose that for $\gamma \in \{\alpha, \beta\}$ there is an event model $M_\gamma \in \mathcal{AM}$ such that
$\vdash [M_\gamma] \gamma$
and $\vdash \langle M_\gamma \rangle \gamma \iff \exists \gamma$.
Then there exists a finite tree-like event model $M \in \mathcal{AM}$ such that $\vdash [M]\phi$ and
$\vdash \langle M \rangle \phi \iff \exists \phi$.
\end{lemma}

\begin{lemma}
Let $\phi = \phi \land \cover \Gamma \in \mathcal{L}_{\otimes\forall}$.
Suppose that for $\gamma \in \Gamma$ there is an event model $M_\gamma \in \mathcal{AM}$ such that
$\vdash [M_\gamma] \gamma$ and $\vdash \langle M_\gamma \rangle \gamma \iff \exists \gamma$.
Then there exists a finite tree-like event model $M \in \mathcal{AM}$ such that $\vdash [M]\phi$ and
$\vdash \langle M \rangle \phi \iff \exists \phi$.
\end{lemma}
...
Then there exists a finite tree-like event model $M \in \mathcal{AM}$ such that
