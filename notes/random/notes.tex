...
This indicates that from $M^1$, an agent in $B$ considers $M^2$ a possible
update.\\
\\
...
Then $M' = ((\ldots((P \mapsto_{A^1} K^1) \to_{A^2} K^2) \to_{A^3} \ldots)\to_{A^n} K^n)$.
...
\end{thm}
To prove this we shall show that any pointed subtree of $M$, $M'$, whose point
is at its root can be constructed by public announcements, insanity event models
and $\to$.
We will induct on the subtrees of $M$.\\
\\
Suppose $M'$ is a leaf node of $M$, $m$.
...
\\
  %TODO: need tree like event model definition to establish claim
