...
\begin{figure}[ht!]
	...
	\end{tikzpicture}
	\caption{$\evM^1$, the model we would have generated using Corollary
    \ref{approximatingBRestricted}. Here, $\evpr^1(\eta_k) = p$ for every $k \leq n$.}
	\label{beforefigure}
	\end{subfigure}
	~
	\begin{subfigure}[b]{.45\textwidth}
	...
	\end{tikzpicture}
	\caption{$M^2$, the public announcement of $p$ where $\evpr(\eta) = p$.}
	\label{publicAnnounce}
	\end{subfigure}
	\caption{We compare models $\evM^1$ and $\evM^2$. Although $\evM^1$ and $\evM^2$
		fulfill the postcondition of $\phi$ after their execution, $\evM^2$ is much smaller than $\evM^1$.}
