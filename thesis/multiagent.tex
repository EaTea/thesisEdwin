...
\section{Technical preliminaries} \label{subsec:Multi:techPrelim}
...
\ref{figure:exampleTree}), and we make an important note that insanity models are the most basic kind of tree-like action models.
...
\centering
\FIXME
\caption[Example tree]{Example tree}
...
Let $\evM_\evr = (\evS, \evR, \evpr, \evT)$ be a finite forest action model.
We say the pointed action model $\evM' = (\evS', \evR', \evpr', \evr')$ is a {\em subtree} of $\evM$ if
\begin{itemize}
	...
	\item $\evM'$ is a finite tree action model
\end{itemize}
\end{defn}
...
\begin{figure}
	\centering
	\FIXME
	\caption[Example subtrees]{Example of subtrees, probably subfigures}
...
\begin{figure}
	...
	\label[Example leaves]{figure:exampleLeaves}
\end{figure}
...
Let $\evM^1_{\evT^1}, \evM^2_{\evT^2} \in \forestClass$ be two disjoint forest action models.
...
This is necessary to show that our operations will construct all possible trees, and we will also show that
using a tree any kind of action model can be constructed, up to $n$-bisimilarity.
...
\\
Let $\evM' = ((\evS', \evR', \evpr'), \evr')$ be an arbitrary subtree of $\evM$.
...
\]
We will show that $\evM^K$ and $\evM'$ are equivalent action models.\\
\\
...
We can form $\evM'$ using insanity action models and $\to$, and thus any arbitrary subtree of $\evM'$
...
\begin{enumerate}
	\item each point $\evt \in \evT'$ is the root of a finite tree action model $\evM^\evt_\evt = (\evS^\evt, \evR^\evt, \evpr^\evt, \evt)$ \label{forestConOne}
...
\\
Suppose $\evs \neq \evs'$.
Since $\evM^{\evs'}_{\evs'} \in \treeClass$ by the definition of a finite tree action model (Definition \ref{finTree}) we have that there is some
...
\\
Since $\evM^{\evs'}_{\evs'} \in \treeClass$ by the definition of a finite tree action model (Definition
...
\section{Comparison to sequent calculi for epistemic planning} \label{subsec:multi:sequentCalc}

So far, we have been particularly interested in constructing action models.
We have shown that we can approximate action models according to what
post-conditions are true.
This was through the construction of a particular action model that fulfilled
the post-conditions that we aimed to fulfil.\\
\\
This approach is powerful in the sense that we are guaranteed to construct an action model
...
\begin{figure}
	...
	\end{subfigure}
	\caption[Comparing announcements against the models we generate]{We compare models $\evM^1_{\evs_0}$ (Subfigure \ref{TheLongChain}) and $\evM^2_\evs$
		(Subfigure \ref{TheShortAnnounce}). Although $\evM^1_{\evs_0}$ and
		$\evM^2_\evs$
		fulfil the post-condition of $\phi$ after their execution, $\evM^2_\evs$ is much smaller than
		$\evM^1_\evs$.}
...
We have shown methods to build action models, but only concentrated on one singular action model to
...
\begin{figure}
	...
	\end{subfigure}
	\caption[Comparing minimal changes against generated action models]{We compare models $\evM^1_{\evs_0}$ (Subfigure \ref{TheStupidTree}) and $\evM^2_\evT$
...
In particular, the inference relation $\models^2$ describes a set of action models that fulfil its
properties.
We will informally say that $\mathcal{M}2(\phi,\phi'')$ is the group of action models such that, for a $\phi,
	 \phi'' \in \lang$, if
\[
...
\\
We can now reason about our updates in their action model form.
