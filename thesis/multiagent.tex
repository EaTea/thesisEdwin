...
\begin{figure}[ht!]
	...
	\end{subfigure}
	\caption{We compare models $\evM^1_{\evs_0}$ and $\evM^2_\evs$. Although $\evM^1_{\evs_0}$ and
		$\evM^2_\evs$
...
\begin{figure}[ht!]
	...
	\label{publicAnnounce}
	\end{subfigure}
	\caption{We compare models $\evM^1_{\evs_0}$ and $\evM^2_\evT$. It is clear that after executing
...
We will informally say that $\mathcal{M}2(\phi,\phi'')$ is the group of event models such that, for a $\phi,
	 \phi'' \in \lang$, if $\evM_\evs \in \mathcal{M}2(\phi,\phi'') \iff (\forall \phi' \in \langActEx
...
$\phi$ is a postcondition of $\phi$, for all models $\krMo_s \in \kripkeClass$, we have that
\[
	\forall \psi \in \lang \text{ such that } \krMo_s \otimes \evM'_{\evt'} \models \psi \implies
	\krMo_s \otimes \evM_\evt \models \psi
\]
\end{defn}

We have given two examples of minimal change above --- for $\phi = \Box^n p$ the minimal change is
the public announcement of $\phi$.
Similarly, for $\psi = \Box(\alpha \lor \beta)$, the disjoint union of the public announcement of
$\alpha$ and the public announcement of $\beta$ is a minimal change.\\
\\
...
\begin{enumerate}
	\item what is the smallest event model $\evM_\evt$ (in terms of possible actions) such that
	$\evM_\evt \in \mathcal{M}2(\top,\sqex{\evM_\evt}\phi \land (\anex{\evM_\evt} \iff \exists
				\phi))$?
	\item what is the event model $\evM_\evt$ representing a minimal change such that
	$\evM_\evt \in \mathcal{M}2(\top,\sqex{\evM_\evt}\phi \land (\anex{\evM_\evt} \iff \exists
				\phi))$?
\end{enumerate}
