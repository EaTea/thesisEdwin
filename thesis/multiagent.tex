...
\begin{figure}
    ...
    \path[edge]
      ...
      (3) edge node {$B$} (5)
          edge node {$C$} (7);
	\end{tikzpicture}
  \caption{An example action model that we generate to fulfill the condition
  $\Box_C \Box_B \Box_C p \Box_B \Box_C \Box_B p$, for $p$ some propositional
  atom and $C, B$ are agents in $A$.
  Here, $\evM_\evs$ is a finite tree action model, with $\evpr(\evs) = \exists
  (\Box_C \Box_B \Box_C p \Box_B \Box_C \Box_B p)$.}
  ...
  see that the succinctness and shortness of our constructed action models are
  somewhat lacking, since the same epistemic goals are being achieved by a much
  smaller action model.}
...
\\
Two open questions that we believe have been elicited from our construction
framework are
\begin{enumerate}
