\section{Multi-agent epistemic goal achievement in $\langArbAct$ --- filler text added 3 September
	2013}
...
\begin{figure}[ht!]
	...
	\begin{tikzpicture}[-,>=stealth',shorten >=1pt,auto,node distance=2cm,
			...
			\node[vertex] (3) [right of=2] {$\evs_{n-1}$};
			\node[vertex] (4) [right of=3] {$\evs_{n}$};
			\path[edge]
						(1) edge node {$A$} (2)
						(3) edge node {$A$} (4);
      \draw[loosely dotted] (2) -- (3);
	\end{tikzpicture}
	\caption{$\evM^1_\evs$, the model we would have generated using Corollary
    \ref{approximatingBRestricted}. Here, $\evpr^1(\evs_k) = p$ for every $k \leq n$.}
	\label{beforefigure}
	\end{subfigure}
	~
	\begin{subfigure}[b]{.45\textwidth}
	...
	\end{subfigure}
	\caption{We compare models $\evM^1_\evs$ and $\evM^2_\evs$. Although $\evM^1_\evs$ and
		$\evM^2_\evs$
...
Similarly, let $\evM^2$ be \FIXME.
This time, we note that $\evM^2$ \FIXME.
