...
These axioms hold for our modalities of knowledge, and Fagin et. al \cite{fagin1995reasoning} show
...
%
%\begin{defn}
%	The operator $\Diamond_a \neg H$ stands for ``Agent $a$ considers $\neg H$ to be possible".
...
%				\theta)$, known as {\bf K}
%		\item $\Box \phi \implies \phi$, known as {\bf \neg H}
...
%\end{itemize}
%
%If we replaced {\bf \neg H} with an axiom {\bf D} which states $\Box \phi
...
%{\bf \neg H} causes the logic to allow agents to hold inconsistent beliefs.
...
%\neg H$ are both false.
%We note that $\Box_b (H \lor \neg H)$ is true and $\Box_a \Box_b (H \lor \neg H)$ is
%true.\\
%\\
%{\bf K}, {\bf \neg H}, {\bf 4} and {\bf 5} allow us to model epistemic state and
...
\caption{Our model after $b$ returns and is unsure about whether $a$ has looked
	at the coin.
$t$ and $s$ are worlds where $a$ has not cheated while $t'$ and $s'$
...
In Figure \ref{amkripkefigure}, we can say that $H$ is true at $\{s,s'\}$ and $\neg H$ is
