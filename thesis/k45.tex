...
\begin{proof}
  ...
  Then $\evM^k_{\evs'}$ is the unwound $k$-model of $\evM_\evs$.
	...
	some event model that is $k$-bisimilar to $\evM'_\evs$.\\
	\\
  ...
  $\EM(\to,\sqcup,I)$, $\evM^{k'}_{\evs''}$ that is $k$-bisimilar to $\evM^k_{\evs'}$.\\
	\\
	Let $k = 0$.
  ...
  \evpr'(\evs')$.
	Then $\evM^{0'}_{\evs'} \sim_0 \evM^0_{\evs'}$.\\
	\\
  ...
  which is $0$-bisimilar to $\evM^0_{\evs'}$.
  ...
  It remains to be shown that $\evM^{k+1'}_{\evs'} \sim_{k+1} \evM^{k+1}_{\evs'}$.
%  \\
%  We note that from our base case, $\evpr^{k+1'}(\evs') \iff
%  \evpr^{k+1}(\evs')$, satisfying {\bf pre}.
%  We simply need to show that for all $a \in A$, $(k+1)$-forth-$a$ and
%  $(k+1)$-back-$a$ both hold.\\
%  \\
%  We first show {\bf $(k+1)$-forth-$a$}.
%  If $a \notin A'$ then $\evs' \evR^{k+1'}_a$ is empty.
%  Therefore, let $a \in A'$, and let us consider $\evs' \evR^{k+1'}_a \evt'$
%  We need to then show that there is some $\evt \in \evR^{k+1}_a$ such that
%  $\evM^{k+1'}_{\evt'} \sim_k \evM^{k+1}_{\evt}$.\\
%  \\
%  We already have, from earlier arguments, that we constructed some model
%  $\evM^{k'}_{\evt'}$ that is bisimilar to $\evM^k_{\evt}$, where $\evt \in
%  \evT'$ was some point in $\evM'_{\evT'}$, the unwound model of $\evM_\evT$.
%  Furthermore we used this model within our construction.
%  We thus claim that $\evM^{k+1'}_{\evt'} \sim_k \evM^{k+1}_{\evt}$.\\
%  \\
%  It is clear that to show this, we need only prove that any point $\evu' \in
%  \evt' \evR^{k+1'}_{a'}$ is $k-1$-bisimilar to some point $\evu \in \evS'$.
\end{proof}
