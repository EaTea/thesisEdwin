...
\end{lemma}
\begin{proof}
	We will show that for $k \leq n$, and for any $\evs \in \evS$ we can construct an event model
  $\evM''_\evs$ such that $\evM''_\evs$ is $k$-bisimilar to $\evM'_\evs$.
	Showing that this condition holds is amenable to a proof by induction over $k$.\\
  \\
  Let $k = 0$, and let $\evs \in \evS$.
  Consider the insanity event model, $\evM^0_{\evs^0} =
  ((\evS^0,\evR^0,\evpr^0),\evs)$ where
  \[
