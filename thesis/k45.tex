...
entirely consistent.
However, composing action models to preserve {\bf D} and {\bf T} is nontrivial, due to the
structures required being no longer tree-like in their overall shape.
Due to these technical difficulties of preserving {\bf D} and {\bf T}, we will
...
\end{lemma}
\begin{proof}
	See Appendix \ref{proof:lemma:k45preserved} for the proof.
\end{proof}
...
\begin{defn} \label{defn:k45:considers}
	...
	all $b$ in $B$, for all $\evt \in \evT'$, $\evt \evR'_b = \evT'$ (that is, all action points in
	$\evT'$ appear indistinguishable to agents in $B$).
...
We say that an action model is {\em composable} in $\EM(\{\to, \sqcup, I\}, \insaneClass)$ if
corresponds with some term in the language.
...
We first show that $\EM(\{\to,\sqcup,I\}, \insaneClass)$ will construct $\AXKFF$ action models.
...
$\EM(\{\to,\sqcup,I\}, \insaneClass)$.
We will now show that we can actually construct any unwound $n$-model using $\EM(\{\to,\sqcup,I\},
\insaneClass)$, up to $n$-bisimilarity.
...
It is clear that if $\evp = \evs$ then this would be the unwound $2$-model of $\evM_\evs$.}
  ...
  $\EM(\{\to,\sqcup,I\}, \insaneClass)$, $\evM^{k'}_{\evp}$ that is bisimilar to $\evMM^k_{\evs'}$.\\
	\\
	Let $k = 0$.
