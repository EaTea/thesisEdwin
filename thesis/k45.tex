...
If we instead consider approximating a finite submodel of the unwound model
...
\begin{lemma} \label{lemma:unwoundModelBisimilarConstruct}
  ...
  There is some construction using $\EM(\to,\sqcup,I)$ that is $n$-bisimilar to
  $\evM'_{\evT'}$.
\end{lemma}
\begin{proof}
  We will show this through induction over $n$.
  Our induction hypothesis is as follows: suppose $\evs' \in \evS'$ where $\evs'
  = \evp \evs$, for some $\evp \in \evS^\ast$.
  We can construct a model $\evM^k_{\evT_k} =
  ((\evS_k,\evR_k,\evpr_k),\evT_k)$ that is $k$-bisimilar to
  $\evM^k_{\evs'}$, for $0 \leq k \leq n$.\\
  \\
  As a base case, suppose $k = 0$.
  Let $\evM^k_{\evs^k} =
  ((\{\evs^k\},\varnothing,\{(\evs^k,\evpr(\evs))\}),\{\evs^k\}) \in
  \insaneClass$.
  It is clear from the definition of the unwound model that $\evpr_k(\evs^k) =
  \evpr(\evs) = \evpr'(\evs')$.
  Then $\evM^k_{\evs^k} \sim_0 \evM'_{\evs'}$.\\
  \\
  Suppose our induction hypothesis holds for some $k$.
  We now show that it holds for $k+1$.
  Let us construct $B \subseteq A$ and $C \subseteq A$, where $a \in B \iff
  \evR'_a \evs' = \varnothing$, and $C = A \setminus B$.
  This is equivalent to saying that $a \in C \iff \evR'_a \evs' \neq
  \varnothing$.\\
  \\
  Firstly, suppose that $a \in B$.
  Then for all $\evt' \in \evs' \evR'_a$, we have that there is some model that
  we can construct, $\evM^k_{\evt^k}$, such that $\evM^k_{\evt^k} \sim_k
  \evM'_{\evt'}$.\\
  \\
  We can now form our intermediate model, $\evM^b_{\evT^b} = \bigsqcup_{t' \in \evs' \evR'_b} \evM^k_{\evt^k}$.
  Consider the insanity event model $\evM^{in}_{\evs''} =
  ((\{\evs''\},\varnothing,\{(\evs'',\evpr(\evs))\}),\{\evs''\})$.
  This is clearly $0$-bisimilar to $\evM'_{\evs'}$.
  Then for $b_1,b_2,\ldots,b_i \in B$, we can form the model
  \[
    \evM^B_{\evT^B} = ((\ldots((\evM^{in}_{\evs''}
      \to_{b_1} \evM^{b_1}_{\evT^{b_1}}) \to_{b_2}
        \evM^{b_2}_{\evT^{b_2}}) \to_{b_3} \ldots) \to_{b_i}
      \evM^{b_i}_{\evT^{b_i}})
  \]

  Now, we consider agents in $C$.
  Now, let $c \in C$.
  For all $\evt' \in \evs' \evR'_c$, we have some model $\evM^k_{\evt^k}$ that
  we can construct such that $\evM^k_{\evt^k} \sim_k \evM'_{\evt'}$.
  Now, we can form another intermediate model $\evM^c_{\evT^c} =
  I_c(\bigsqcup_{t' \in \evs' \evR'_c} \evM^k_{\evt^k})$.
\end{proof}
