...
\begin{proof}
	...
	\begin{itemize}
		...
		\item $\evR^k_a = \varnothing$ if $\evR'_a \evs$ is nonempty
		\item $\evpr^k = \evpr'$ restricted to $\evS^k$
	\end{itemize}

	It is clear that when $\evs' \in \evT'$, then $\evM^k_{\evs'}$ is the unwound $k$-model of
	$\evM_\evs$ (since $\evR'_a \evs'$ is always empty).
	It will thus be sufficient to use this representation of a subtree and show that we can construct
	something that is $k$-bisimilar to $\evs' \in \evT'$.\\
	\\
	To show this we will induct over $k$, from 0 to $n$.
	We will consider, for a point $\evs' \in \evS'$, the submodel of $\evM'_{\evT'}$,
	$\evM^k_{\evs'}$ and show that we can construct a model using $\EM(\to,\sqcup,I)$ that is
	$k$-bisimilar to this submodel.\\
	\\
	Let $k = 0$.
	Let $\evM^0_{\evs'} = ((\evS^0,\evR^0,\evpr^0),\{\evs'\}) \in \insaneClass$ where
	\begin{itemize}
		\item $\evS^0 = \{\evs'\}$
	...
	As $\evpr^0(\evs') = \evpr(\evs) = \evpr'(\evs')$ then $\evpr^0(\evs') \iff \evpr(\evs)$.
	Then $\evpr^0(\evs') \sim_0 \evpr'(\evs')$.\\
	\\
