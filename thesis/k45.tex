...
\begin{theorem} \label{thm:nBisimAnyK45EventModel}
  Suppose $\evM_\evT = ((\evS,\evR,\evpr),\evT)$ is a $\AXKFF$ multi-pointed
  event model.
...
We restate this result in terms of goal achievement in Corrolary
\ref{corr:k45GoalAchievement}.

\begin{corr} \label{corr:k45GoalAchievement}
  Suppose $\phi \in \lang$ is some epistemic goal with $\MD(\phi) = n$ for $n$
  ...
  There is some event model $\evM'_{\evT'}$ constructible by $\EM(\t,\sqcup,I)$
...
This final result (, in terms of goal achievement, states that if we have an 
