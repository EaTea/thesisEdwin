...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
    ...
    \node[vertex] (4) [below right of=2] {$\omega$};
    \path[edge]
          (1) edge node {} (2)
          (2) edge node {} (3)
          (2) edge node {} (4);
...
Furthermore, by \axFi for any formula $k \in \lang$ such that $\krMo'_\alpha \models
\Diamond k$, we have that $\krMo'_\alpha \models \Box \Diamond k$.
That is, for any action point that $\alpha$ was connected to (say $\alpha$), every subsequent
action point (in this case $\gamma$ and $\omega$)}
\label{k45model}
\end{subfigure}
\caption{We compare two Kripke models, $\krMo$ (Subfigure \ref{kmodel}) and
  $\krMo'$ (Subfigure \ref{k45model}).
The effect of \axFo and \axFi on $\krMo'$ is evident when comparing Subfigure
\ref{kmodel} and Subfigure \ref{k45model}.
For example, we can see that \axFi forces the action points to all be related to each other.}
\label{k45VsKModels}
\end{figure}
...
This is easily contrasted with $\evM_\alpha$, which has an outgoing $B$-edge,
     and $\evM_\omega$ which has a reflexive $B$-edge.}
