...
need to specify models of updates, which we call {\em action models} or {\em action models}.

\begin{defn} \label{evModel}
	Let $L$ be a logical language.
	Let $F = (S, R)$ be a frame, and $A$ a set of agents.
	An {\em action model} (also known as an {\em action model}) $\aMod{\evMo = (S, R, pre)}$ with preconditions defined on $\mathcal{L}$ consists of
	\begin{itemize}
...
Furthermore, in literature both of the terms action models and action models are used to describe
the same concept, and we use them interchangeably throughout this paper to refer to action models.
