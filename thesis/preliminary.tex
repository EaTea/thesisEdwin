...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
    ...
    \node[vertex] (1) {$\evs$};
    \node[vertex] (2) [above right of=1] {$\evt$};
		\node[vertex] (3) [below right of=1] {$\evu$};
    \path[edge]
          (1) edge node {$b$} (2)
							edge node {$b$} (3)
              edge [loop left] node {$a,b$} (1)
...
Such Kripke semantics for modal logic are well-known.
...
Next, we build upon our logics to introduce the concepts of updates and refinements as modal logics.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
      thick]
		...
		\node[vertex] (1) {($s, \evt$)};
		\node[vertex] (2) [right of=1] {($t, \evu$)};
		\node[vertex] (3) [below of=1] {($s, \evs$)};
		\node[vertex] (4) [below of=2] {($t, \evs$)};
    \path[edge]
          (1) edge node {$b$} (2)
							edge node {$b$} (3)
							edge node {$b$} (4)
