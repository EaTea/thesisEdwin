...
This second update is interesting in that its announcement causes itself to be
false.
...
\\
van Ditsmarsch provides operations to construct dynamic Epistemic formulae, and
...
Thus, van Ditsmarsch presents a way to describe complex actions, and indeed we can
...
worlds models that were discussed in Section \ref{Epistemic}. \citep{baltag1998lpa}
\footnote{Action Models resemble the possible worlds models discussed in Section
\ref{Epistemic}, but there were some discrepancies which meant they were not
entirely equivalent. Aucher resolves this and restores the symmetry between the
models, allowing us to reason about the actions within Action Models. \citep{aucher09revisited}}
          ...
          (2) edge node {} (1)
              ...
              edge [loop right] node {$A,B$} (2)
					(3) edge [loop below] node {$A,B$} (3)
							...
							edge node {} (2);
\end{tikzpicture}
\caption{An example of an Action Model; in this situation, the update models $B$
...
Both Relational Epistemic Actions \citep{hoek2008dynamic} and Action Models
\citep{baltag1998lpa} describe a larger range of Epistemic Updates for our game of heads and tails.
...
inside a syntax \citep{baltag2005programs}.
...
Action Models.
If $\phi$ is a formula, Hales' determines if any Action Model $\alpha$ exists
such that executing $\alpha$ on any model ensures that $\phi$ is true.
If such an $\alpha$ exists then Hales shows a method by which an equivalent
Action Model can be generated.
...
heads and tails by employing logics that have become progressively more
expressive.
